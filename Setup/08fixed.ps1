# PowerShell Script to Create Static Files (CORRECTED)
...
# Custom Site CSS - Using here-string with single quotes to avoid variable expansion
$siteCssContent = @'
...
$siteCssContent | Out-File -FilePath "src\CloudMigrationTool.Web\wwwroot\css\site.css" -Force -Encoding UTF8

Write-Host "Creating JavaScript Files..." -ForegroundColor Cyan

# Custom Site JavaScript - Using here-string with single quotes and proper escaping
$siteJsContent = @'
...
$siteJsContent | Out-File -FilePath "src\CloudMigrationTool.Web\wwwroot\js\site.js" -Force -Encoding UTF8

Write-Host "Downloading Bootstrap and jQuery..." -ForegroundColor Cyan

try {
    # Create CDN download for Bootstrap CSS
    $bootstrapCss = Invoke-RestMethod -Uri "https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" -ErrorAction Stop
    $bootstrapCss | Out-File -FilePath "src\CloudMigrationTool.Web\wwwroot\css\bootstrap.min.css" -Encoding UTF8
    Write-Host "Bootstrap CSS downloaded successfully" -ForegroundColor Green
}
catch {
    Write-Warning "Failed to download Bootstrap CSS: $($_.Exception.Message)"
    Write-Host "Please download Bootstrap CSS manually from: https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" -ForegroundColor Yellow
}

try {
    # Create CDN download for Bootstrap JS
    $bootstrapJs = Invoke-RestMethod -Uri "https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js" -ErrorAction Stop
    $bootstrapJs | Out-File -FilePath "src\CloudMigrationTool.Web\wwwroot\js\bootstrap.bundle.min.js" -Encoding UTF8
    Write-Host "Bootstrap JS downloaded successfully" -ForegroundColor Green
}
catch {
    Write-Warning "Failed to download Bootstrap JS: $($_.Exception.Message)"
    Write-Host "Please download Bootstrap JS manually from: https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js" -ForegroundColor Yellow
}

try {
    # Create CDN download for jQuery
    $jqueryJs = Invoke-RestMethod -Uri "https://code.jquery.com/jquery-3.6.0.min.js" -ErrorAction Stop
    $jqueryJs | Out-File -FilePath "src\CloudMigrationTool.Web\wwwroot\js\jquery-3.6.0.min.js" -Encoding UTF8
    Write-Host "jQuery downloaded successfully" -ForegroundColor Green
}
catch {
    Write-Warning "Failed to download jQuery: $($_.Exception.Message)"
    Write-Host "Please download jQuery manually from: https://code.jquery.com/jquery-3.6.0.min.js" -ForegroundColor Yellow
}
...
try {
    ...
    Write-Warning "Failed to create favicon: $($_.Exception.Message)"
}
